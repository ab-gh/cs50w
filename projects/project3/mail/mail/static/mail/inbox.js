...
function load_mailbox(mailbox) {
	...
	.then(emails => {
		...
		unarchiveBtn.forEach(function(currentBtn){
  			currentBtn.addEventListener('click', function(event) {
				event.stopPropagation();
				fetch(`/emails/${this.dataset.id}`, {
					method: 'PUT',
		...
		readBtn.forEach(function(currentBtn){
  			currentBtn.addEventListener('click', function(event) {
				event.stopPropagation();
				fetch(`/emails/${this.dataset.id}`, {
					method: 'PUT',
		...
		const unreadBtn = document.querySelectorAll('.unread-btn')
		unreadBtn.forEach(function(currentBtn){
  			currentBtn.addEventListener('click', function(event) {
				event.stopPropagation();
				fetch(`/emails/${this.dataset.id}`, {
					method: 'PUT',
...
class Archive extends React.Component {
	render() {
		if (this.props.state == true) {
			return(
				<div data-id={this.props.id} className="unarchive-btn">
					<svg width="1.5em" height="1.5em" viewBox="0 0 16 16" class="mb-1 bi bi-inbox" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
			...
			return(
				<div data-id={this.props.id} className="archive-btn">
					<svg width="1.5em" height="1.5em" viewBox="0 0 16 16" class="mb-1 bi bi-archive" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
...
class Read extends React.Component {
	render() {
		if (this.props.state == true) {
			return(
				<div data-id={this.props.id} className="unread-btn">
			...
			return(
				<div data-id={this.props.id} className="read-btn">
...
class Mail extends React.Component {
	...
	render() {
		...
		for (let i = 0; i < this.props.emails.length; i++) {
			...
			content.push(
				<a key={item.id} data-id={item.id} className={`list-group-item list-group-item-action ${ read }`}>
					...
					<div className="d-flex w-100 justify-content-between">
						...
						<div>
							<Read state={item.read} id={item.id} />
							<br />
							<Archive state={item.archived} id={item.id} />
						</div>
					</div>
